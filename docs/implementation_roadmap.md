...
10. Live promotion gates
   ...
   - The 2026-05-11 market/side filtered v8 run preserved execution quality but did not solve adverse selection: 182 signals, observed fill-rate `1.0`, synthetic-vs-observed gap `0.0`, adverse selection `0.9807692307692307`, and lower coverage than the unfiltered run. The resulting recommendation is `REJECT_MARKET_SIDE_RISK_FILTER`; the next research loop needs stronger adverse-selection features rather than repeating the same market/side filter.
