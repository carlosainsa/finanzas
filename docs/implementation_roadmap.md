...
10. Live promotion gates
   ...
   - `fill_toxicity_v1` now exports bucketed toxic segments by market, asset, side, strategy/model, spread bucket, and timing bucket. The generated `blocked_segments.json` is still research-only, but `execution_probe_v9` can load it for the next dry-run through `PREDICTOR_BLOCKED_SEGMENTS_PATH` so rejected toxic segments are skipped deterministically instead of interpreted manually.
