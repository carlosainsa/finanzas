...
10. Live promotion gates
   ...
   - Runtime-touch A/B now writes `runtime_touch_ab_root_cause.json` for gate failures, preflight failures, and completed A/B comparisons. It classifies terminal outcomes into `NO_SIGNALABLE_ASSETS`, `SIGNALS_ZERO`, `REPORTS_ZERO`, `ERRORS_RISK_EXECUTOR`, `UNMATCHED_QUOTES`, `TOXIC_FILLS`, `LOW_FILL_RATE`, `SYNTHETIC_OPTIMISM`, or `ADVERSE_SELECTION`.
   - Auto-route remains strict by default: `--run-selected-ab` refuses selected ladder commands that bypass the signalability gate through hybrid backfill or explicit skip flags unless the operator passes `--allow-gate-bypass` for a deliberate research diagnostic.
