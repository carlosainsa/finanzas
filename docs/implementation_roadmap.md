...
10. Live promotion gates
   ...
   - `scripts/run_runtime_touch_cycle.sh` is the preferred next operator path: fresh dry-run capture, `runtime_touch_ranking_v1`, `selection_source=runtime_touch`, dry-run observation with explicit `execution_probe_v11`, at-touch diagnostic, and `execution_probe_touch_comparison_v1`. Do not introduce a new predictor profile until the runtime-touch universe proves that market/timing selection can generate fills.
