...
10. Live promotion gates
   ...
   - The 2026-05-13 fresh v10 cycle generated a strong base window (`10786` signals, `21570` reports), promoted two executable segments offline, then produced a 30-minute sparse runtime observation with `orderbook=977`, `signals=0`, and `reports=0`. The cycle now records this as `sparse_probe_no_signals` / `DO_NOT_PROMOTE` instead of a generic crash. Next: re-rank or widen v10 selection by runtime activity/coverage before repeating; do not change live gates or global quote aggression from this result.
