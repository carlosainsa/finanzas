...
CYCLE_TIMESTAMP="${CYCLE_TIMESTAMP:-execution-probe-v11-cycle-$(date -u +%Y%m%dT%H%M%SZ)}"
...
SELECTION_SOURCE="${EXECUTION_PROBE_UNIVERSE_SELECTION_SOURCE:-${EXECUTION_PROBE_SELECTION_SOURCE:-touch_probability}}"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v11_cycle.sh --universe-duckdb PATH [--baseline-report-root PATH] [--comparison-report-roots CSV] [--duration-seconds N] [--print-plan]

Runs the full execution_probe_v11 research cycle:
touch-probability ranking -> dry-run observation -> profile comparison -> next decision.
...
if [[ "$SELECTION_SOURCE" != "touch_probability" ]]; then
  echo "execution_probe_v11 selection source must be touch_probability" >&2
  exit 64
fi
...
OBSERVATION_COMMAND=(
  "$ROOT_DIR/scripts/run_execution_probe_v11_observation.sh"
...
print(json.dumps({
    "script": "scripts/run_execution_probe_v11_cycle.sh",
    ...
    "delegates_to": [
        ...
        "src.research.touch_probability_ranking",
    ...
    "outputs": {
        "touch_probability_ranking": f"{run_root}/execution_probe_universe_selection/touch_probability_ranking/touch_probability_ranking.json",
        ...
        "cycle_summary": f"{run_root}/execution_probe_v11_cycle_summary.json",
    },
...
RUN_ROOT="$RUN_ROOT" PROFILE=execution_probe_v11 \
...
if [[ "$observation_status" != "0" && "$observation_status" != "20" ]]; then
  echo "execution_probe_v11 observation failed with status $observation_status" >&2
...
if [[ ${#PROFILE_ARGS[@]} -gt 0 && ! -f "$REPORT_ROOT/profile_observation_comparison.json" ]]; then
...
if [[ -d "$REPORT_ROOT" ]]; then
  PYTHONPATH=python-service python3 -m src.research.asset_execution_decision \
    ...
    --json \
    > "$REPORT_ROOT/asset_execution_decision.stdout.json" || true
fi
...
summary = {
    "report_version": "execution_probe_v11_cycle_summary_v1",
    "can_execute_trades": False,
    "execution_mode": "dry_run",
    ...
    "touch_probability_ranking_path": str(
        run_root
        / "execution_probe_universe_selection"
    ...
    "asset_execution_decision_path": str(
        report_root / "asset_execution_decision" / "asset_execution_decision.json"
    ),
    "recommendation": decision.get("recommendation", "DO_NOT_PROMOTE"),
    "next_step": decision.get(
        "next_step",
        "Review v11 observation evidence before changing live trading gates.",
    ),
}
run_root.mkdir(parents=True, exist_ok=True)
(run_root / "execution_probe_v11_cycle_summary.json").write_text(
