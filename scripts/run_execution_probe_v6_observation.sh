...
FRACTION_SELECTION_PATH="${PREDICTOR_EXECUTION_PROBE_V6_FRACTION_SELECTION_PATH:-}"

usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v6_observation.sh --universe-selection PATH [--fraction-selection PATH] [--duration-seconds N] [--print-plan]

Runs a reproducible multi-market execution_probe_v6 dry-run observation. V6 is
research-only and quotes closer to touch than v5 to test actual executability.

Options:
  --universe-selection PATH   execution_probe_universe_selection.json.
  --fraction-selection PATH   optional execution_probe_v6_fraction_selection.json.
...
if universe.get("profile") not in {"execution_probe_v5", "execution_probe_v6"}:
...
if fraction_path:
    ...
    if fraction.get("profile") != "execution_probe_v6":
        raise SystemExit("fraction selection must target execution_probe_v6")
PY

export EXECUTION_MODE="dry_run"
export DISABLE_MARKET_WS="false"
export PREDICTOR_STRATEGY_PROFILE="execution_probe_v6"
export PREDICTOR_QUOTE_PLACEMENT="${PREDICTOR_QUOTE_PLACEMENT:-near_touch}"
export PREDICTOR_EXECUTION_PROBE_V6_NEAR_TOUCH_MAX_SPREAD_FRACTION="${PREDICTOR_EXECUTION_PROBE_V6_NEAR_TOUCH_MAX_SPREAD_FRACTION:-1.0}"
export PREDICTOR_EXECUTION_PROBE_V6_OFFSET_TICKS="${PREDICTOR_EXECUTION_PROBE_V6_OFFSET_TICKS:-0}"
export EXECUTION_PROBE_UNIVERSE_SELECTION_PATH="$UNIVERSE_SELECTION_PATH"
if [[ -n "$FRACTION_SELECTION_PATH" ]]; then
  export PREDICTOR_EXECUTION_PROBE_V6_FRACTION_SELECTION_PATH="$FRACTION_SELECTION_PATH"
fi
...
print(json.dumps({
    "script": "scripts/run_execution_probe_v6_observation.sh",
    ...
    "predictor_execution_probe_v6_near_touch_max_spread_fraction": float(os.environ["PREDICTOR_EXECUTION_PROBE_V6_NEAR_TOUCH_MAX_SPREAD_FRACTION"]),
    "predictor_execution_probe_v6_offset_ticks": int(os.environ["PREDICTOR_EXECUTION_PROBE_V6_OFFSET_TICKS"]),
    "execution_probe_universe_selection_path": os.environ["EXECUTION_PROBE_UNIVERSE_SELECTION_PATH"],
    "predictor_execution_probe_v6_fraction_selection_path": os.environ.get("PREDICTOR_EXECUTION_PROBE_V6_FRACTION_SELECTION_PATH"),
