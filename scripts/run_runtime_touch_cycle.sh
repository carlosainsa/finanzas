...
print(json.dumps({
    ...
    "outputs": {
        ...
        "execution_failure_diagnostics": f"{observation_report_root}/execution_failure_diagnostics.json",
...
PYTHONPATH=python-service python3 -m src.research.execution_failure_diagnostics \
  --report-root "$OBSERVATION_REPORT_ROOT" \
  --output "$OBSERVATION_REPORT_ROOT/execution_failure_diagnostics.json" \
  > "$OBSERVATION_REPORT_ROOT/execution_failure_diagnostics.stdout.json"
...
summary = {
    ...
    "execution_failure_diagnostics": read_json(
        observation_report_root / "execution_failure_diagnostics.json"
    ),
