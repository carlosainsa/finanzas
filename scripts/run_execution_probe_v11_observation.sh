...
FRACTION_SELECTION_PATH="${PREDICTOR_EXECUTION_PROBE_V11_FRACTION_SELECTION_PATH:-}"

usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v11_observation.sh --universe-selection PATH [--fraction-selection PATH] [--duration-seconds N] [--print-plan]

Runs a reproducible execution_probe_v11 dry-run observation. V11 is research-only
and restricts the universe to markets ranked by observed/future touch probability.

Options:
  --universe-selection PATH   execution_probe_universe_selection.json.
  --fraction-selection PATH   optional execution_probe_v11_fraction_selection.json.
...
if universe.get("profile") != "execution_probe_v11":
    raise SystemExit("universe selection must target execution_probe_v11")
if universe.get("source_report_version") != "touch_probability_ranking_v1":
    raise SystemExit("execution_probe_v11 requires touch_probability_ranking_v1")
touch_filter = universe.get("touch_probability_filter")
if not isinstance(touch_filter, dict) or not touch_filter.get("enabled"):
    raise SystemExit("execution_probe_v11 requires touch_probability_filter")
if not universe.get("market_asset_ids"):
    raise SystemExit("execution_probe_v11 requires at least one asset id")
...
if fraction_path:
    ...
    if fraction.get("profile") != "execution_probe_v11":
        raise SystemExit("fraction selection must target execution_probe_v11")
PY

export EXECUTION_MODE="dry_run"
export DISABLE_MARKET_WS="false"
export PREDICTOR_STRATEGY_PROFILE="execution_probe_v11"
...
export SIGNAL_REJECTION_PROFILES="${SIGNAL_REJECTION_PROFILES:-execution_probe_v11}"
export SIGNAL_REJECTION_BASELINE_PROFILE="${SIGNAL_REJECTION_BASELINE_PROFILE:-execution_probe_v11}"
export SIGNAL_REJECTION_CANDIDATE_PROFILE="${SIGNAL_REJECTION_CANDIDATE_PROFILE:-execution_probe_v11}"
export EXECUTION_PROBE_UNIVERSE_SELECTION_PATH="$UNIVERSE_SELECTION_PATH"
if [[ -n "$FRACTION_SELECTION_PATH" ]]; then
  export PREDICTOR_EXECUTION_PROBE_V11_FRACTION_SELECTION_PATH="$FRACTION_SELECTION_PATH"
fi
...
print(json.dumps({
    "script": "scripts/run_execution_probe_v11_observation.sh",
    ...
    "predictor_execution_probe_v11_fraction_selection_path": os.environ.get("PREDICTOR_EXECUTION_PROBE_V11_FRACTION_SELECTION_PATH"),
