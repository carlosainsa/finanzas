...
FRACTION_SELECTION_PATH="${PREDICTOR_EXECUTION_PROBE_V8_FRACTION_SELECTION_PATH:-}"

usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v8_observation.sh --universe-selection PATH [--fraction-selection PATH] [--duration-seconds N] [--print-plan]

Runs a reproducible multi-market execution_probe_v8 dry-run observation. V8 is
research-only and quotes at touch inside stricter market/timing filters to test
whether v7's one-tick unmatched orders can become observed fills.

Options:
  --universe-selection PATH   execution_probe_universe_selection.json.
  --fraction-selection PATH   optional execution_probe_v8_fraction_selection.json.
...
if universe.get("profile") != "execution_probe_v8":
    raise SystemExit("universe selection must target execution_probe_v8")
...
if fraction_path:
    ...
    if fraction.get("profile") != "execution_probe_v8":
        raise SystemExit("fraction selection must target execution_probe_v8")
PY

export EXECUTION_MODE="dry_run"
export DISABLE_MARKET_WS="false"
export PREDICTOR_STRATEGY_PROFILE="execution_probe_v8"
export PREDICTOR_QUOTE_PLACEMENT="${PREDICTOR_QUOTE_PLACEMENT:-near_touch}"
export PREDICTOR_EXECUTION_PROBE_V8_NEAR_TOUCH_MAX_SPREAD_FRACTION="${PREDICTOR_EXECUTION_PROBE_V8_NEAR_TOUCH_MAX_SPREAD_FRACTION:-1.0}"
export PREDICTOR_EXECUTION_PROBE_V8_OFFSET_TICKS="${PREDICTOR_EXECUTION_PROBE_V8_OFFSET_TICKS:-0}"
export EXECUTION_PROBE_UNIVERSE_SELECTION_PATH="$UNIVERSE_SELECTION_PATH"
if [[ -n "$FRACTION_SELECTION_PATH" ]]; then
  export PREDICTOR_EXECUTION_PROBE_V8_FRACTION_SELECTION_PATH="$FRACTION_SELECTION_PATH"
fi
...
print(json.dumps({
    ...
    "predictor_execution_probe_v8_near_touch_max_spread_fraction": float(os.environ["PREDICTOR_EXECUTION_PROBE_V8_NEAR_TOUCH_MAX_SPREAD_FRACTION"]),
    "predictor_execution_probe_v8_offset_ticks": int(os.environ["PREDICTOR_EXECUTION_PROBE_V8_OFFSET_TICKS"]),
    "execution_probe_universe_selection_path": os.environ["EXECUTION_PROBE_UNIVERSE_SELECTION_PATH"],
    "predictor_execution_probe_v8_fraction_selection_path": os.environ.get("PREDICTOR_EXECUTION_PROBE_V8_FRACTION_SELECTION_PATH"),
