...
CYCLE_TIMESTAMP="${CYCLE_TIMESTAMP:-execution-probe-v8-cycle-$(date -u +%Y%m%dT%H%M%SZ)}"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v8_cycle.sh --universe-duckdb PATH [--baseline-report-root PATH] [--duration-seconds N] [--universe-selection-source candidate_market_ranking|fillability] [--market-timing-filter none|future_touch] [--print-plan]

Runs the full execution_probe_v8 research cycle:
universe selection -> at-touch dry-run observation -> profile comparison -> next decision.
...
OBSERVATION_COMMAND=(
  "$ROOT_DIR/scripts/run_execution_probe_v8_observation.sh"
...
RUN_ROOT="$RUN_ROOT" PROFILE=execution_probe_v8 \
...
if [[ "$observation_status" != "0" && "$observation_status" != "20" ]]; then
  echo "execution_probe_v8 observation failed with status $observation_status" >&2
...
(run_root / "execution_probe_v8_cycle_summary.json").write_text(
