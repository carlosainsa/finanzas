...
CYCLE_TIMESTAMP="${CYCLE_TIMESTAMP:-execution-probe-v6-cycle-$(date -u +%Y%m%dT%H%M%SZ)}"
...
usage() {
  cat <<'EOF'
Usage: scripts/run_execution_probe_v6_cycle.sh --universe-duckdb PATH [--baseline-report-root PATH] [--duration-seconds N] [--market-timing-filter none|future_touch] [--print-plan]

Runs the full execution_probe_v6 research cycle:
...
OBSERVATION_COMMAND=(
  "$ROOT_DIR/scripts/run_execution_probe_v6_observation.sh"
...
print(json.dumps({
    "script": "scripts/run_execution_probe_v6_cycle.sh",
    ...
    "outputs": {
        ...
        "cycle_summary": f"{run_root}/execution_probe_v6_cycle_summary.json",
    },
...
RUN_ROOT="$RUN_ROOT" PROFILE=execution_probe_v6 \
...
if [[ "$observation_status" != "0" && "$observation_status" != "20" ]]; then
  echo "execution_probe_v6 observation failed with status $observation_status" >&2
...
summary = {
    "report_version": "execution_probe_v6_cycle_summary_v1",
    "can_execute_trades": False,
    "execution_mode": "dry_run",
...
(run_root / "execution_probe_v6_cycle_summary.json").write_text(
