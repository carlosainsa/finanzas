...
CYCLE_TIMESTAMP="${CYCLE_TIMESTAMP:-execution-probe-v10-cycle-$(date -u +%Y%m%dT%H%M%SZ)}"
...
SELECTION_SOURCE="${EXECUTION_PROBE_UNIVERSE_SELECTION_SOURCE:-${EXECUTION_PROBE_SELECTION_SOURCE:-executable_segments}}"
...
OBSERVATION_COMMAND=(
  "$ROOT_DIR/scripts/run_execution_probe_v10_observation.sh"
...
print(json.dumps({
    "script": "scripts/run_execution_probe_v10_cycle.sh",
    ...
    "outputs": {
        ...
        "segment_opportunity_ranking": f"{run_root}/execution_probe_universe_selection/segment_opportunity_ranking/segment_opportunity_ranking.json",
        "allowed_segments": f"{run_root}/execution_probe_universe_selection/segment_opportunity_ranking/allowed_segments.json",
        ...
        "cycle_summary": f"{run_root}/execution_probe_v10_cycle_summary.json",
    },
...
RUN_ROOT="$RUN_ROOT" PROFILE=execution_probe_v10 \
...
if [[ "$observation_status" != "0" && "$observation_status" != "20" ]]; then
  echo "execution_probe_v10 observation failed with status $observation_status" >&2
...
summary = {
    ...
    "segment_opportunity_ranking_path": str(run_root / "execution_probe_universe_selection" / "segment_opportunity_ranking" / "segment_opportunity_ranking.json"),
    "allowed_segments_path": str(run_root / "execution_probe_universe_selection" / "segment_opportunity_ranking" / "allowed_segments.json"),
