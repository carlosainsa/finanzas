...
def test_execution_probe_v9_observation_print_plan_is_safe_and_pinned(
    tmp_path: Path,
) -> None:
    universe = tmp_path / "execution_probe_universe_selection.json"
    universe.write_text(
    ...
    plan = json.loads(completed.stdout)
    assert plan["script"] == "scripts/run_execution_probe_v9_observation.sh"
    assert plan["delegates_to"] == "scripts/run_pre_live_dry_run.sh"
    assert plan["execution_mode"] == "dry_run"
...
def test_execution_probe_v9_cycle_print_plan_is_safe_and_pinned(
    ...
    plan = json.loads(completed.stdout)
    assert plan["script"] == "scripts/run_execution_probe_v9_cycle.sh"
    assert plan["can_execute_trades"] is False
    assert plan["execution_mode"] == "dry_run"
...
def test_runtime_touch_ab_cycle_print_plan_is_research_only(tmp_path: Path) -> None:
    ...
    assert plan["min_runtime_signalable_snapshots"] == 3
    ...
    assert plan["runtime_touch_hybrid_backfill"] is False
...
def test_runtime_touch_ab_retry_ladder_print_plan_is_research_only(
    ...
    assert plan["config"]["runtime_touch_freshness_ordering"] == "freshest_first"
...
def test_runtime_touch_discovery_loop_print_plan_is_research_only(
    ...
    completed = subprocess.run(
        [
            ...
            str(tmp_path / "family-memory.json"),
            "--exploration-rate",
            "0.25",
            "--print-plan",
        ],
