...
def test_execution_probe_v7_is_less_aggressive_than_v6_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v7")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    monkeypatch.setattr(settings, "execution_mode", "dry_run")
    monkeypatch.setattr(settings, "app_env", "development")
    monkeypatch.setattr(settings, "predictor_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v7_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v7_min_depth", 1.0)
    monkeypatch.setattr(
        settings,
    ...
    assert signal is not None
    assert signal.price == 0.49
    assert signal.strategy == "passive_spread_capture_execution_probe_near_touch_v7"
    assert (
        signal.feature_version
...
def test_execution_probe_v7_rejects_live_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v7")
    monkeypatch.setattr(settings, "execution_mode", "live")
...
def test_execution_probe_v7_rejects_live_fraction_selection(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    selection_path = tmp_path / "selection.json"
        ...
        json.dumps(
            {
                ...
                "near_touch_max_spread_fraction": 0.85,
                "decision_policy": "offline_fraction_selection_only",
                "can_execute_trades": True,
            }
        ),
...
def test_execution_probe_v7_blocklist_uses_v7_model_version(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v7")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    ...
    blocklist = SegmentBlocklist(
        [
            BlockedSegment(
                ...
                asset_id="123",
                side="BUY",
                model_version="passive_spread_capture_execution_probe_near_touch_v7",
...
def test_execution_probe_v8_quotes_at_touch_with_timing_profile_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v8")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    monkeypatch.setattr(settings, "execution_mode", "dry_run")
    monkeypatch.setattr(settings, "app_env", "development")
    monkeypatch.setattr(settings, "predictor_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v8_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v8_min_depth", 1.0)
    monkeypatch.setattr(settings, "predictor_execution_probe_v8_max_top_changes", 4)
    monkeypatch.setattr(
        settings,
    ...
    assert signal is not None
    assert signal.price == 0.50
    assert signal.strategy == "passive_spread_capture_execution_probe_near_touch_v8"
    assert (
        signal.feature_version
...
def test_execution_probe_v8_rejects_live_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v8")
    monkeypatch.setattr(settings, "execution_mode", "live")
...
def test_execution_probe_v8_rejects_live_fraction_selection(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    selection_path = tmp_path / "selection.json"
...
def test_execution_probe_v8_blocklist_uses_v8_model_version(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v8")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    ...
    blocklist = SegmentBlocklist(
        [
            BlockedSegment(
                ...
                reason="quote_timing_probe_failed",
            )
        ]
    )
...
def test_execution_probe_v9_is_less_aggressive_than_v8_and_versioned(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v9")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    monkeypatch.setattr(settings, "execution_mode", "dry_run")
    monkeypatch.setattr(settings, "app_env", "development")
    monkeypatch.setattr(settings, "predictor_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v9_min_confidence", 0.50)
    monkeypatch.setattr(settings, "predictor_execution_probe_v9_min_depth", 1.0)
    monkeypatch.setattr(settings, "predictor_execution_probe_v9_max_top_changes", 2)
    monkeypatch.setattr(
        settings,
    ...
    assert signal is not None
    assert signal.price == 0.495
    assert signal.strategy == "passive_spread_capture_execution_probe_near_touch_v9"
    assert (
        signal.feature_version
...
def test_execution_probe_v9_rejects_live_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v9")
    monkeypatch.setattr(settings, "execution_mode", "live")
...
def test_execution_probe_v9_rejects_live_fraction_selection(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    selection_path = tmp_path / "selection.json"
        ...
        json.dumps(
            {
                ...
                "near_touch_max_spread_fraction": 0.90,
                "decision_policy": "offline_fraction_selection_only",
                "can_execute_trades": True,
            }
        ),
...
def test_execution_probe_v9_blocklist_uses_v9_model_version(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(settings, "predictor_strategy_profile", "execution_probe_v9")
    monkeypatch.setattr(settings, "predictor_quote_placement", "near_touch")
    ...
    blocklist = SegmentBlocklist(
        [
            BlockedSegment(
                ...
                asset_id="123",
                side="BUY",
                model_version="passive_spread_capture_execution_probe_near_touch_v9",
                reason="fill_toxicity_probe_failed",
            )
        ]
    )
