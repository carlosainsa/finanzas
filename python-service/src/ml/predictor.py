...
from src.ml.execution_probe_selection import (
    ...
    load_execution_probe_v11_fraction_selection,
    load_execution_probe_v12_fraction_selection,
)
...
EXECUTION_PROBE_V8_MODEL_VERSION = "passive_spread_capture_execution_probe_v8"
EXECUTION_PROBE_V8_FEATURE_VERSION = "orderbook_top_of_book_execution_probe_v8"
EXECUTION_PROBE_V8_NEAR_TOUCH_MODEL_VERSION = (
    "passive_spread_capture_execution_probe_near_touch_v8"
)
EXECUTION_PROBE_V8_NEAR_TOUCH_FEATURE_VERSION = (
    "orderbook_top_of_book_execution_probe_near_touch_v8"
)
EXECUTION_PROBE_V9_MODEL_VERSION = "passive_spread_capture_execution_probe_v9"
EXECUTION_PROBE_V9_FEATURE_VERSION = "orderbook_top_of_book_execution_probe_v9"
EXECUTION_PROBE_V9_NEAR_TOUCH_MODEL_VERSION = (
    "passive_spread_capture_execution_probe_near_touch_v9"
)
...
EXECUTION_PROBE_V11_MODEL_VERSION = "passive_spread_capture_execution_probe_v11"
EXECUTION_PROBE_V11_FEATURE_VERSION = "orderbook_top_of_book_execution_probe_v11"
EXECUTION_PROBE_V11_NEAR_TOUCH_MODEL_VERSION = (
    "passive_spread_capture_execution_probe_near_touch_v11"
)
EXECUTION_PROBE_V11_NEAR_TOUCH_FEATURE_VERSION = (
    "orderbook_top_of_book_execution_probe_near_touch_v11"
)
EXECUTION_PROBE_V12_MODEL_VERSION = "passive_spread_capture_execution_probe_v12"
EXECUTION_PROBE_V12_FEATURE_VERSION = "orderbook_top_of_book_execution_probe_v12"
EXECUTION_PROBE_V12_NEAR_TOUCH_MODEL_VERSION = (
    "passive_spread_capture_execution_probe_near_touch_v12"
)
EXECUTION_PROBE_V12_NEAR_TOUCH_FEATURE_VERSION = (
    "orderbook_top_of_book_execution_probe_near_touch_v12"
)
...
def quote_price_for_buy(best_bid: float, best_ask: float) -> tuple[float, str, str]:
    ...
    if placement == "passive_bid":
        ...
        if profile.name == "execution_probe_v8":
            return (
                best_bid,
                EXECUTION_PROBE_V8_MODEL_VERSION,
                EXECUTION_PROBE_V8_FEATURE_VERSION,
            )
        ...
        if profile.name == "execution_probe_v11":
            return (
                best_bid,
                EXECUTION_PROBE_V11_MODEL_VERSION,
                EXECUTION_PROBE_V11_FEATURE_VERSION,
            )
        if profile.name == "execution_probe_v12":
            return (
                best_bid,
                EXECUTION_PROBE_V12_MODEL_VERSION,
                EXECUTION_PROBE_V12_FEATURE_VERSION,
            )
    ...
    if profile.name == "execution_probe_v11":
        offset = settings.predictor_execution_probe_v11_offset_ticks * tick_size
    if profile.name == "execution_probe_v12":
        offset = settings.predictor_execution_probe_v12_offset_ticks * tick_size
    cap = best_ask - offset
...
def strategy_profile() -> StrategyProfile:
    ...
    if profile == "execution_probe_v8":
        validate_execution_probe_allowed()
        selection = load_execution_probe_v8_fraction_selection(
            settings.predictor_execution_probe_v8_fraction_selection_path,
            default_fraction=(
                settings.predictor_execution_probe_v8_near_touch_max_spread_fraction
            ),
        )
        return StrategyProfile(
            name=profile,
            min_confidence=max(
                settings.predictor_min_confidence,
                settings.predictor_execution_probe_v8_min_confidence,
            ),
            near_touch_max_spread_fraction=selection.near_touch_max_spread_fraction,
            min_depth=settings.predictor_execution_probe_v8_min_depth,
            max_top_changes=settings.predictor_execution_probe_v8_max_top_changes,
            top_change_window_ms=settings.predictor_execution_probe_v8_top_change_window_ms,
            risk_filters_enabled=True,
            min_signal_interval_ms=(
                settings.predictor_execution_probe_v8_min_signal_interval_ms
            ),
        )
    if profile == "execution_probe_v9":
        validate_execution_probe_allowed()
        selection = load_execution_probe_v9_fraction_selection(
            settings.predictor_execution_probe_v9_fraction_selection_path,
            default_fraction=(
                settings.predictor_execution_probe_v9_near_touch_max_spread_fraction
            ),
        )
        return StrategyProfile(
            name=profile,
            min_confidence=max(
                settings.predictor_min_confidence,
                settings.predictor_execution_probe_v9_min_confidence,
            ),
            near_touch_max_spread_fraction=selection.near_touch_max_spread_fraction,
            min_depth=settings.predictor_execution_probe_v9_min_depth,
            max_top_changes=settings.predictor_execution_probe_v9_max_top_changes,
            top_change_window_ms=settings.predictor_execution_probe_v9_top_change_window_ms,
            risk_filters_enabled=True,
            min_signal_interval_ms=(
    ...
    if profile == "execution_probe_v11":
        validate_execution_probe_allowed()
        selection = load_execution_probe_v11_fraction_selection(
            settings.predictor_execution_probe_v11_fraction_selection_path,
            default_fraction=(
                settings.predictor_execution_probe_v11_near_touch_max_spread_fraction
            ),
        )
        return StrategyProfile(
            name=profile,
            min_confidence=max(
                settings.predictor_min_confidence,
                settings.predictor_execution_probe_v11_min_confidence,
            ),
            near_touch_max_spread_fraction=selection.near_touch_max_spread_fraction,
            min_depth=settings.predictor_execution_probe_v11_min_depth,
            max_top_changes=settings.predictor_execution_probe_v11_max_top_changes,
            top_change_window_ms=settings.predictor_execution_probe_v11_top_change_window_ms,
            risk_filters_enabled=True,
            min_signal_interval_ms=(
                settings.predictor_execution_probe_v11_min_signal_interval_ms
            ),
        )
    if profile == "execution_probe_v12":
        validate_execution_probe_allowed()
        selection = load_execution_probe_v12_fraction_selection(
            settings.predictor_execution_probe_v12_fraction_selection_path,
            default_fraction=(
                settings.predictor_execution_probe_v12_near_touch_max_spread_fraction
            ),
        )
        return StrategyProfile(
            name=profile,
            min_confidence=max(
                settings.predictor_min_confidence,
                settings.predictor_execution_probe_v12_min_confidence,
            ),
            near_touch_max_spread_fraction=selection.near_touch_max_spread_fraction,
            min_depth=settings.predictor_execution_probe_v12_min_depth,
            max_top_changes=settings.predictor_execution_probe_v12_max_top_changes,
            top_change_window_ms=settings.predictor_execution_probe_v12_top_change_window_ms,
            risk_filters_enabled=True,
            min_signal_interval_ms=(
                settings.predictor_execution_probe_v12_min_signal_interval_ms
            ),
        )
...
def validate_near_touch_allowed() -> None:
    ...
    if (
        ...
        raise ValueError(
            "predictor execution probe v8 near-touch max spread fraction must be between 0 and 1"
        )
    if settings.predictor_execution_probe_v8_offset_ticks < 0:
        raise ValueError("predictor execution probe v8 offset ticks must be non-negative")
    if (
        not 0
        <= settings.predictor_execution_probe_v9_near_touch_max_spread_fraction
        <= 1
        ...
        raise ValueError(
            "predictor execution probe v9 near-touch max spread fraction must be between 0 and 1"
        )
    ...
    if (
        ...
        raise ValueError(
            "predictor execution probe v11 near-touch max spread fraction must be between 0 and 1"
        )
    if settings.predictor_execution_probe_v11_offset_ticks < 0:
        raise ValueError("predictor execution probe v11 offset ticks must be non-negative")
    if (
        not 0
        <= settings.predictor_execution_probe_v12_near_touch_max_spread_fraction
        <= 1
        ...
        raise ValueError(
            "predictor execution probe v12 near-touch max spread fraction must be between 0 and 1"
        )
    if settings.predictor_execution_probe_v12_offset_ticks < 0:
        raise ValueError("predictor execution probe v12 offset ticks must be non-negative")
