...
def restricted_blocklist_diagnostics(
    ...
    restricted_input_rows = [
        row
        for row in rows
        if typed_dict(row.get("sources")).get("expected_restricted_input") is True
    ]
    candidate_blocked = [
        row
        for row in rows
    ...
    efficacy = restricted_blocklist_effectiveness(
        restricted_input_rows=restricted_input_rows,
        unexpected_rows=unexpected_new,
        candidate_blocked_rows=candidate_blocked,
    )
    return {
        "report_version": "restricted_blocklist_diagnostics_v1",
        "status": status,
        "summary": {
            "restricted_input_blocklist_segments": len(expected),
            "baseline_generated_blocklist_segments": len(baseline),
            "candidate_generated_blocklist_segments": len(candidate),
            "restricted_input_observed_in_candidate_generated_segments": len(
                candidate & expected
            ),
            "restricted_input_missing_from_candidate_generated_segments": len(
                expected - candidate
            ),
            "candidate_generated_unexpected_segments": len(candidate - expected),
        ...
        "restricted_input_blocklist": {
            "description": "Segments explicitly supplied to the restricted run through PREDICTOR_BLOCKED_SEGMENTS_PATH.",
            **diagnostic_bucket(restricted_input_rows),
        },
        "candidate_generated_blocklist": {
            "description": "Segments generated by the candidate run pre-live promotion analysis after the restricted observation.",
            **diagnostic_bucket(candidate_blocked),
        },
    ...
    *,
    ...
    return {
        **segment_key_to_dict(key),
        ...
        "sources": {
            "restricted_input_blocklist": in_expected,
            "baseline_generated_blocklist": in_baseline,
            "candidate_generated_blocklist": in_candidate,
...
def diagnostic_metric_rollup(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "pnl": sum(
            numeric_values_from_rows(rows, "pnl")
        ),
...
def restricted_blocklist_effectiveness(
    *,
    restricted_input_rows: list[dict[str, object]],
    unexpected_rows: list[dict[str, object]],
    candidate_blocked_rows: list[dict[str, object]],
) -> dict[str, object]:
    input_metrics = diagnostic_metric_rollup_using_source(
        restricted_input_rows,
        source="baseline_metrics",
    )
    migrated_risk_metrics = diagnostic_metric_rollup_using_source(
        unexpected_rows,
        source="candidate_metrics",
    )
    generated_metrics = diagnostic_metric_rollup_using_source(
        candidate_blocked_rows,
        source="candidate_metrics",
    )
    return {
        "report_version": "restricted_blocklist_effectiveness_v1",
        "status": effectiveness_status(
            restricted_input_rows=restricted_input_rows,
            unexpected_rows=unexpected_rows,
        ),
        "baseline_restricted_input_contribution": input_metrics,
        "candidate_unexpected_blocked_risk": migrated_risk_metrics,
        "candidate_generated_blocklist_contribution": generated_metrics,
        "risk_migration": {
            "unexpected_blocked_segments": len(unexpected_rows),
            "unexpected_signals": migrated_risk_metrics["signals"],
            "unexpected_pnl": migrated_risk_metrics["pnl"],
            "unexpected_max_drawdown": migrated_risk_metrics["max_drawdown"],
        },
        "net_effect": restricted_blocklist_net_effect(
            input_metrics=input_metrics,
            migrated_risk_metrics=migrated_risk_metrics,
            unexpected_rows=unexpected_rows,
        ),
...
def diagnostic_metric_rollup_using_source(
    rows: list[dict[str, object]],
    *,
    source: str,
) -> dict[str, object]:
    ...
    for row in rows:
        source_rows.append(
            {
                "candidate_metrics": typed_dict(row.get(source)),
                "baseline_metrics": {},
            }
        )
    return diagnostic_metric_rollup(source_rows)


def restricted_blocklist_net_effect(
    *,
    input_metrics: dict[str, object],
    migrated_risk_metrics: dict[str, object],
    unexpected_rows: list[dict[str, object]],
) -> dict[str, object]:
    expected_signals = numeric_or_none(input_metrics.get("signals")) or 0.0
    unexpected_signals = numeric_or_none(migrated_risk_metrics.get("signals")) or 0.0
    expected_pnl = numeric_or_none(input_metrics.get("pnl")) or 0.0
    unexpected_pnl = numeric_or_none(migrated_risk_metrics.get("pnl")) or 0.0
    ratio_value = ratio(unexpected_signals, expected_signals)
    if expected_signals <= 0:
        ...
        verdict = "ineffective"
    return {
        "expected_removed_signals": expected_signals,
        "unexpected_risk_signals": unexpected_signals,
        "unexpected_to_expected_signal_ratio": ratio_value,
        "expected_removed_pnl": expected_pnl,
        "unexpected_risk_pnl": unexpected_pnl,
        "verdict": verdict,
    }
...
def effectiveness_status(
    *,
    ...
    if not restricted_input_rows:
        return "missing_restricted_input_evidence"
    if unexpected_rows:
        return "risk_migration_detected"
    return "restricted_input_isolated"


def effectiveness_interpretation(
    *,
    restricted_input_rows: list[dict[str, object]],
    unexpected_rows: list[dict[str, object]],
) -> list[str]:
    output: list[str] = []
    if restricted_input_rows:
        output.append("baseline_contribution_of_restricted_input_measured")
    else:
        output.append("restricted_input_not_found_in_segment_metrics")
    if unexpected_rows:
        output.append("candidate_generated_new_blocked_segments_after_restriction")
    else:
        output.append("no_candidate_generated_unexpected_blocked_segments")
    return output


def numeric_values_from_rows(
    rows: list[dict[str, object]],
    metric: str,
) -> list[float]:
    values: list[float] = []
...
def restricted_blocklist_source_paths(
    ...
    return {
        "restricted_input_blocklist_path": blocked_segments_path
        if isinstance(blocked_segments_path, str)
        else None,
        "candidate_generated_blocklist_json_path": str(
            candidate_report_root / "pre_live_promotion" / "blocked_segments.json"
        ),
        "candidate_generated_blocklist_parquet_path": str(
            candidate_report_root
            / "pre_live_promotion"
